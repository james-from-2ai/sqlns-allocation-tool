...
def build_sources(book):
    """Provenance for each constant, read from 'Hard-coded Inputs' column D.

    The workbook cites its own numbers in the column beside them. Carrying that
    through means the site can show where a figure came from instead of asking
    the reader to trust it, which matters more than the figure itself.

    Each entry is (key, label cell, value cell, source cell).
    """
    spec = [
        ("mortalityEffect", "B63", "C63", "D63"),
        ("stuntingEffect", "B58", "C58", "D58"),
        ("samEffect", "B53", "C53", "D53"),
        ("anemiaEffect", "B48", "C48", "D48"),
        ("wastage", "B116", "C116", "D116"),
        ("incompleteConsumption", "B117", "C117", "D117"),
        ("dalyDiscount", "B69", "C69", "D69"),
        ("lifeExpectancy", "B70", "C70", "D70"),
        ("anemiaWeight", "B74", "C74", "D74"),
        ("samWeight", "B82", "C82", "D82"),
        ("deathWeight", "B90", "C90", "D90"),
        ("u5Share", "B36", "C36", "D36"),
        ("share6to23", "B37", "C37", "D37"),
        ("sachetsPerCarton", "B6", "C6", "D6"),
        ("sachetsPerYear", "B10", "C10", "D10"),
        ("pricePerSachet", "B5", "C5", "D5"),
        ("ngnPerUsd", "B2", "C2", "D2"),
        ("deliveryCostPerChild", "B25", "C25", "D25"),
    ]
    refs = [r for row in spec for r in row[1:]]
    cells = read_cells(book, "Hard-coded Inputs", refs)

    out = {}
    for key, label_ref, value_ref, source_ref in spec:
        label = cells.get(label_ref)
        source = cells.get(source_ref)
        out[key] = {
            "label": str(label).strip() if label else key,
            "source": str(source).strip() if source else None,
        }

    # Strategy definitions live in C97:C100, described in prose.
    strat = read_cells(book, "Hard-coded Inputs", ["B97", "C97", "B98", "C98",
                                                   "B99", "C99", "B100", "C100"])
    out["_strategyDefinitions"] = [
        {"label": strat.get(f"B{r}"), "description": strat.get(f"C{r}")}
        for r in (97, 98, 99, 100)
    ]
    return out
...
def main():
    ...
    print("reading source citations ...")
    sources = build_sources(book)
    cited = sum(1 for k, v in sources.items() if not k.startswith("_") and v.get("source"))
    print(f"  {cited} constants carry a citation")
